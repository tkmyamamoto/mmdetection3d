    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if args.mode == "eval":
                ...
                xs_val = []
                ys_val = []
                if log_dict[epochs[args.interval - 1]]["mode"][-1] == "train":
                    num_iters_per_epoch = log_dict[epochs[args.interval - 1]]["iter"][
                        -1
                    ]
                else:
                    num_iters_per_epoch = log_dict[epochs[args.interval - 1]]["iter"][
                ...
                for epoch in epochs[args.interval - 1 :: args.interval]:
                    ...
                    if log_dict[epoch]["mode"][-1] == "val":
                        iters_train = iters[:-1]
                        iters_val = iters_train[-1]
                        xs_val.append(iters_val + (epoch - 1) * num_iters_per_epoch)
                        ys_val.append(log_dict[epoch][metric][-1])
                    xs_train.append(
                        np.array(iters_train) + (epoch - 1) * num_iters_per_epoch
                    )
                    ys_train.append(
                        np.array(log_dict[epoch][metric][: len(iters_train)])
                    )
                ...
                plt.plot(
                    xs_train, ys_train, label=legend[i * num_metrics + j], linewidth=0.5
                )
                if args.val:
                    plt.plot(xs_val, ys_val, label="val_loss", linewidth=0.5)
            plt.legend()
...
def add_plot_parser(subparsers):
    ...
    parser_plt.add_argument("--epoch", type=int, default=24)
    parser_plt.add_argument("--val", action="store_true")
    parser_plt.add_argument("--ylim", type=float, nargs=2, default=None)
